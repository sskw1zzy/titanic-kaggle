...
from src.dnn import DNNModel, train_dnn, prepare_dnn_data
...
def evaluate_ensemble_averaging(X, y, X_dnn, seed, n_splits, lgbm_params, dnn_model_params, dnn_train_params, weights=(1, 1, 1)):
    ...
    for train_idx, val_idx in skf.split(X, y):
        ...
        dnn_model = DNNModel(input_size=X_dnn.shape[1], **dnn_model_params)
        loss_fn = nn.BCEWithLogitsLoss()
        optimizer = optim.Adam(dnn_model.parameters(), lr=dnn_train_params['lr'])
        dnn_model = train_dnn(dnn_model, train_loader, loss_fn, optimizer, dnn_train_params['num_epochs'])
        
        dnn_model.eval()
        with torch.no_grad():
            X_val_tensor = torch.tensor(X_dnn_val.astype('float32').values, dtype=torch.float32)
            dnn_probs = torch.sigmoid(dnn_model(X_val_tensor)).squeeze().numpy()
...
def generate_oof_predictions(X, y, X_dnn, seed, n_splits, lgbm_params, knn_params, dnn_model_params, dnn_train_params):
    ...
    for train_idx, val_idx in skf.split(X, y):
        ...
        with torch.no_grad():
            X_val_tensor = torch.tensor(X_dnn_val.astype('float32').values, dtype=torch.float32)
            oof_preds[val_idx, 2] = torch.sigmoid(dnn_model(X_val_tensor)).squeeze().numpy()
...
def predict_ensemble_averaging(X_train, y_train, X_test, X_dnn_train, X_dnn_test, seed, lgbm_params, knn_params, dnn_model_params, dnn_train_params, weights):
    ...
    torch.manual_seed(seed)
    train_loader = prepare_dnn_data(X_dnn_train, y_train, batch_size=dnn_train_params['batch_size'])
    dnn_model = DNNModel(input_size=X_dnn_train.shape[1], **dnn_model_params)
    loss_fn = nn.BCEWithLogitsLoss()
    optimizer = optim.Adam(dnn_model.parameters(), lr=dnn_train_params['lr'])
    dnn_model = train_dnn(dnn_model, train_loader, loss_fn, optimizer, dnn_train_params['num_epochs'])
    ...
    dnn_model.eval()
    with torch.no_grad():
        X_test_tensor = torch.tensor(X_dnn_test.astype('float32').values, dtype=torch.float32)
        dnn_probs = torch.sigmoid(dnn_model(X_test_tensor)).squeeze().numpy()
...
def predict_stacking(X_train, y_train, X_test, X_dnn_train, X_dnn_test, oof_preds, seed, lgbm_params, knn_params, dnn_model_params, dnn_train_params, meta_model_type='linear'):
    ...
    with torch.no_grad():
        X_test_tensor = torch.tensor(X_dnn_test.astype('float32').values, dtype=torch.float32)
        dnn_probs_test = torch.sigmoid(dnn_model(X_test_tensor)).squeeze().numpy()
